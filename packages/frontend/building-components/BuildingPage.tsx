...
export default function BuildingPage({items, currPage, itemLimit}: pageProps) {
    ...
    return (
        <Box w={"full"}>
            {currItems.map((building: any, index: number) => {
                return <BuildingBasicForm key={index} {...building}/>;
            })}
        </Box>
    )
}
